...
class BoardService {
    constructor(boardRepository, projectService) {
        this.boardRepository = boardRepository;
        this.projectService = projectService;
    }
    async createBoard(command) {
        ...
        const board = new board_1.Board(0, command.title, command.project_id);
    ...
    async checkUserAccessToBoard(userId, boardId) {
    ...
    // Обновление доски
    async updateBoard(boardId, command) {
        const board = await this.boardRepository.getBoardById(boardId);
        if (!board) {
            return null;
        }
    ...
    async getProjectByBoardId(boardId, userId) {
        const board = await this.boardRepository.getBoardById(boardId);
        if (!board) {
            return null;
        }
